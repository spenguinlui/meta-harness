...
支柱 12 Human Interface 專門設計 human 介面層（翻譯、粒度、回饋通道），對稱支柱 9 觀測（給工程師 / 系統看）。設計時必區分——builder 看得懂 jargon、human 未必。

## Why 顧問模式而非腳手架

最初直覺是 `meta-harness new <domain>` 產出標準目錄。深思後發現：12 支柱每條都是設計參數而非開關，且彼此耦合，沒有單一最佳搭配。
...
- **12 大支柱（pattern library）** — `docs/pillars.md`
...
**v0.4 — Human Interface 支柱 + 多軸 memory + 飛輪 retrospective**

- ✅ **12 支柱完整**（v0.4 新增 Human Interface — human-facing IO 邊界，對稱支柱 9 system-facing）
