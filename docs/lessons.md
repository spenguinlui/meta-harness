...
## 說明書要畫「agent loop + 人機分工」，不只列任務（figma-to-code 學到）

manual-template 的 Viewer 段原本只有「我想做 X → 這樣說」(任務清單)，沒有一段畫出 **agent 自動循環的迴圈 + 人在哪交棒 / 何時拿回控制**。對「會自己循環」的 target(figma2code 的「寫 code → 視覺檢查 → 改」自動 loop、ai-infra 的 advise 4 架構師辯論 loop)，這是**它最大的特點**，卻被淹在任務表裡——viewer 看完不知道「我交了任務後 AI 到底在自動做什麼、我何時該介入」。
...
## 閘門的成本要對準風險，否則會對「沒犯錯的人」收稅（Stop hook 自驗學到）

軸 13 的 Stop hook 一開始設計成**無條件全跑**：每次收工都跑完整套 scorer 才放行。直覺上「越嚴越好」，實際跑久了露出兩個問題：純諮詢 / 純閱讀的 session（沒動任何檔）照樣被罰跑一次；更糟的是**歸因錯誤**——某次純文件審查被擋下，原因是前輪遺留、且源頭在外部 repo 的失修，讀起來像「你這輪做錯了」。

根因：閘門的**觸發條件**沒跟它想防的**風險**對齊。R-10 要防的是「改了可機驗的東西卻沒驗就交付」，風險由「有沒有改」構成；無條件觸發等於把條件放寬成「有沒有收工」，多出來的部分全是稅。

修正：加指紋 gate（架構檔內容雜湊 vs 上次通過時），改了才驗，沒改靜默放行。兩個關鍵不可省——(1) 指紋**只在通過後才寫**，否則「改壞了不修就再收工一次」會被指紋放行，閘門等於被自己繞過；(2) runtime 產出（log / 報告 / runs/）要排除在指紋外，否則自己的輸出會觸發自己，變成永動的空跑。

通則：**設計閘門時，觸發條件、阻擋條件要分開想**。阻擋條件（drift → exit 2）通常一開始就對；被忽略的是觸發條件，而它決定這個閘門是被當成防護還是被當成噪音——後者遲早被人 `--no-verify` 掉。
