...
source_sessions:
  ...
  - <link to 9 pillars audit session>
...
**Required**: 該 repo 在這條支柱該長什麼樣（基於 Part A）
...
某條 pillar 對該 domain 真不需要時，五個欄位仍要填，Mechanism 寫 `N/A — <reason>`。**不允許整段省略**——強制 reviewer 看見「有意決定不做」vs「忘了想」。
