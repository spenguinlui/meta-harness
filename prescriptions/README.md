...
顧問 session 對 target repo 開始實作**之前**，先在這裡寫一份：
...
- **留痕**：強迫顧問把「為什麼這樣改」寫出來。寫不出來表示思考未到位，不該開始實作（`docs/consultant-flow.md`）。
...
- `prescriptions/`（gitignored，只保留 README）：對某次 target repo 寫檔的「實作理由」摘要
