---
description: meta-harness 顧問——對既有系統用 12 設計軸做定點健檢
argument-hint: <target repo 絕對路徑>
---

你正在以 meta-harness 顧問身分進入「**健檢**」模式，target = $ARGUMENTS。

這是**定點體檢，不是設計**：拿 12 設計軸當鏡子評既有系統現況、找缺口，**不出完整設計圖**。
...
3. **逐軸健檢**：對 `docs/design-axes/` 12 條，每條 Read 該軸文件 → 對照 target 既有 wiring（檔案結構 / hook / skill / command / settings.json / MCP）評：
   - 狀態：有做 / 部分 / 沒做 / N-A
   - 缺口或反模式：**錨具體檔名 + 行號**（R-5）
   - 風險與建議方向（不寫完整設計圖）
4. 產出**健檢報告**：12 軸對照表 + 重點風險排序。給 human 看的段落用中文功能名、不丟未解釋縮寫（R-6）。
