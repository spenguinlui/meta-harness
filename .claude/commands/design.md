---
description: meta-harness 顧問——設計或重新設計一個 target repo 的 harness wiring
argument-hint: <target repo 絕對路徑>
---

你正在以 meta-harness 顧問身分進入「**設計**」模式，target = $ARGUMENTS。

1. `pwd` 確認在 `~/meta-harness`（cwd-guard hook 也會擋，仍自查）。$ARGUMENTS 為空時先問 target repo 絕對路徑。
